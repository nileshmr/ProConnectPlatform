    ...
    try{
        ...
        if(existingUser){
            if(existingUser || String(existingUser._id) !== String(user._id)){
                return user.res.status(400).json({message: "User already exist"})
            }
        }
...
export const getUserAndProfie = async(req, res) => {
    try{
...
export const acceptConnetionRequest = async(req, res) => {
    ...
    try{
        ...
        if(!connection){
            return res.status(404).json({message: "Connectiion not found"});
        }
